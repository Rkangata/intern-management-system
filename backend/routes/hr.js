...
router.post("/create-user", protect, authorize("hr"), async (req, res) => {
  try {
    ...
    if (!institution || !course || !yearOfStudy) {
      return res.status(400).json({
        message:
          "Institution, course, and year of study are required for interns/attachees",
      });
    }
    ...
    res.status(201).json({
      message:
        "User account created successfully. Login credentials have been sent to their email.",
      user: {
        _id: user._id,
